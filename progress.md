...
参考 LMCache Long-Document QA benchmark。
...
1. 将当前 GPU-only prefix cache baseline 固化为对照组。
2. 实现 CPU prefix cache backing store。
3. 在 GPU miss 时查询 CPU prefix cache。
4. CPU hit 时将 KV restore 到 GPU，并跳过对应 prefix prefill。
5. 用同一组 `cascade_tile`、`hot_cold_sharing`、`branching_prefix_sharing` 对比 GPU-only 与 GPU+CPU。
